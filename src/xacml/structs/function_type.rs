...
impl FunctionId {
    ...
    pub (super) fn apply(&self, parameters: &Vec<ExpressionType>, request: &RequestType) -> Result<Vec<Value>, XacmlError> {
        let values_owned = get_values(parameters, request)?;
        let values: Vec<&Value> = values_owned.iter().collect();
        self.apply_function(values)
    }
}

fn get_values(parameters: &Vec<ExpressionType>, request: &RequestType) -> Result<Vec<Value>, XacmlError> {
    Ok(parameters.iter()
